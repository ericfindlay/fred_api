...
// The spec string can be taken from the FRED API docs.
let (req_spec, req) = build_request("series/observations?series_id=CPGRLE01AUQ657N").unwrap();

// Determine whether to request from the cache and/or request from FRED. Successful FRED
// responses are always cached. Responses with other than Ok status will return an error.
